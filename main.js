...
client.once('ready', async () => {
    ...
    fs.createReadStream('./contactos.csv')
        .pipe(csv())
        .on('data', (row) => {
            contacts.push(row);
        })
        .on('end', async () => {
            ...
            for (const row of contacts) {
                ...
                try {
                    ...
                    const videoPath = path.join(__dirname, 'videos', 'video_demo.mp4'); // Reemplaza 'video_demo.mp4' con el nombre del archivo de video
